...
rgb_mapping = {'black':	(0,0,0),'white':	(255,255,255),'red':(255,0,0),'lime':(0,255,0),'blue':(0,0,255),'yellow':	(255,255,0),'cyan':(0,255,255),'magenta':(255,0,255),'silver':(192,192,192),'gray':(128,128,128),'maroon':(128,0,0),'olive':(128,128,0),'green':(0,128,0),'purple':(128,0,128),'navy':(0,0,128)}
...
async def data_analysis(data,current_price):
    ...
    if (highest_price-lowest_price) >0:
        relative_percent = ((highest_price-current_price)/(highest_price-lowest_price)) * 100
...
async def send_messages(file_name, msg):
    if msg:
        if file_name:
            ...
            await client.send_file('Online Looters: One-Stop shop for every deal',f'data\\{file_name}' , caption=msg,link_preview=False)
        else:
            print('Sending msg without image')
            await client.send_message('Online Looters: One-Stop shop for every deal',msg,link_preview=False)
    else:
...
async def amazon_store(url):
    ...
    if not ASIN:
        ...
        print('>>',short_url)
        page_title = page_source.find('div',
             {'class': lambda x: x 
                       and x.endswith('page-title')
             }
            )
        if page_title:
        ...
        category = page_source.find(attrs={"selected" : "selected"})['value'].split('=')[-1].upper()
        ...
        if coupon:
            ...
            coupon=''
        print('coupon : ',coupon)
        product_rank = page_source.select_one('#productDetails_detailBullets_sections1 tr:nth-child(3) td')
        if product_rank:
            product_rank = product_rank.text
            product_rank = re.findall(r'#\d+','\n\n#228 in Computers & Accessories (See Top 100 in Computers & Accessories)\n\n#3 in Tablets\n\n\n')[-1]
            product_rank = int(re.findall(r'\d+',product_rank)[0])
        else:
            product_rank = ''
        print('Rank : ',product_rank)
        current_price = page_source.find(id= lambda value:value and value.startswith('priceblock') and value.endswith('price'))
        if current_price:
        ...
        if additional_offers:
            ...
            additional_offers = ' | '.join(set(re.findall(r'No Cost EMI|Exchange Offer|Bank Offer',additional_offers)))
        else:
            additional_offers = ''
        print('Additional Offers : ',additional_offers)
        data = price_history.collect(url)
        if data:
            regular_price, file_name, rating, price_drop_chances = await data_operation(current_price, data)
...
async def handler(event):
    ...
    ## Price History
    url_list +=  [price_history.translate2OriginalUrl(i) for i in url_list if 'pricehistory' in i]
    amzn_url_list = [requests.get(i).url for i in url_list if ('amzn' in i or 'amazon' in i)]
    ...
    print(f'The operation completed for {event.chat_id}', 'Time elapse : ',(datetime.now()-start).seconds)
